  ...
  <div>
    <el-collapse v-model="activeNames" v-loading="loading">
      <el-collapse-item name="3">
        ...
        <div class="el-upload-list__item-name" v-for="(file, index) in fileList" :key="index">
          <i class="el-icon-document"></i>
          <span>{{file.name}}</span>
          <el-button type="text" size="mini"
                              @click='onRemoveFileList(index)'>删除</el-button>
        </div>
        <el-table :data='viewFileList' style='width: 100%' v-show="action != 'add'">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onRemoveFileList: function (index) { // 移除文件
      ...
      onViewFileList(index, row) { // 预览
        ...
        // 文件流访问路径
        let url = window.config(process.env.NODE_ENV).FILE_PREVIEW_URL + `sys/sysSeting/getFile/` + fileId + `?token=` + token
        window.open(window.config(process.env.NODE_ENV).KK_FILE_URL + encodeURIComponent(Base64.encode(url)));
      },
    ...
    watch: {
      'objectId': function(newVal, oldVal) {
        //修复BUG 521：切换数据的表单页，上传的历史记录会在其他个页面显示
        this.fileList = []  // 上传附件列表
        if (!validatenull(this.objectId)) {
          this.getSysFileAll()
        }
