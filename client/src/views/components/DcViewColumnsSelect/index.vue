<template>
  <div>
    ...
    <div  style="text-align:left" >
      <div v-for="(column, index) in handlerVModel" :key='column.order'>
...
export default {
  ...
  computed: {
    handlerVModel() {
      if (validatenull(this.vModel)) return []
      const vModel = Object.values(this.vModel).filter(item => {
        return Object.prototype.toString.call(item) === '[object Object]'
      })
      return vModel.sort(function(newVal, oldVal){ return newVal.order - oldVal.order })
    }
  },
  watch:{
    value: {
