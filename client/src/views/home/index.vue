  ...
  <div class="layout-container" :class="[layout.style , layout.fixedHeader? 'fixed-header' : '', isCollapse? 'is-collapse':'']">
    ...
    <el-container class="body-container" :style="">
      ...
      <el-main class="main" :style="{backgroundColor: settings.backgroundColor}">
        ...
        <div class="main-content fix-main-content">
...
export default {
  ...
  data() {
    return {
      ...
      webSocketUrl: window.config(process.env.NODE_ENV).WEB_SOCKET_URL,
      websocket:null,
      ...
      if(this.userId){
        try {
          if ('WebSocket' in window) {
            ...
            this.websocket = new WebSocket(wsProtocol+window.location.host+this.webSocketUrl+this.userId);
            this.initWebSocket()
          }
        }catch (e) {
          this.reconnect();
