  ...
  methods: {
    ...
    async pageInit() {
      ...
      try {
        ...
        // 数据权限: 角色sys_role
        this.pushDataPermissions(this.search.params, this.$route.meta.routerId, this.tableId)
        let [listRoleRespData, listPermissionRespData] = await Promise.all([
          listRolePage(this.search),
          listResourcePermission(this.$route.meta.routerId)
        ])
        if (listRoleRespData.code == 100 && listPermissionRespData.code == 100) {
          this.tableDataTotal = listRoleRespData.data.total
          this.tableData = listRoleRespData.data.rows ? listRoleRespData.data.rows : []
