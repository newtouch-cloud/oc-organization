        ...
        <el-row>
          ...
          <el-row gutter="0" type="flex" justify="end" align="top" class="dc-el-row_ElRow">
            <el-tooltip disabled class="dc-el-tooltip_Button">
              <el-button type="primary" icon="el-icon-share" v-on:click="synchronizationData">同步数据</el-button>
            </el-tooltip>
            ...
            >
              ...
              <template slot-scope="scope">
                ...
                <OperationIcon
                  ...
                  type="danger"
                  v-show="permission.remove && (!(scope.row.id == currentUser.id))"
                  content="删除"
                  placement="top"
...
import {synchronizationUser} from '@/api/databus/databus'
...
export default {
  ...
  methods: {
    ...
    synchronizationData(){
      //同步数据的逻辑 先访问开发平台 获取到当前租户拥有的应用，然后选择应用进行数据同步
      synchronizationUser()
        .then((responseData) => {
          if (responseData.code == 100) {
            this.showMessage({
              type: 'success',
              msg: '后台正在开始同步当前租户用户主数据,后续可查看日志'
            })
          } else {
            this.showMessage(responseData)
          }
    ...
    onCreate() {
      ...
      let row = {
        company: {
          ...
          tenant: this.parentTable.tenant,
        }
      }
