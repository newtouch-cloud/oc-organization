...
import {listCompanyAppPage, deleteCompanyApp, remotePage} from '@/api/org/companyApp'
  ...
  data() {
    return {
      ...
      tableColumns: {
        142: {
          title: '应用',
          field: 'tenantAppVersion.applicationVersion.app.name',
          visible: true,
          order: 0
        },
        143: {
          title: '应用版本',
          field: 'tenantAppVersion.applicationVersion.name',
          visible: true,
          order: 1
        },
        128: {
          title: '租户应用版本.数据库IP',
          field: 'tenantAppVersion.dbIp',
          visible: true,
          order: 2
        },
        129: {
          title: '租户应用版本.数据库端口',
          field: 'tenantAppVersion.dbPort',
          visible: true,
          order: 3
        },
        130: {
          title: '租户应用版本.数据库名',
          field: 'tenantAppVersion.dbName',
          visible: true,
          order: 4
        },
        131: {
          title: '租户应用版本.数据库用户名',
          field: 'tenantAppVersion.dbUsername',
          visible: true,
          order: 5
        },
        132: {
          title: '租户应用版本.数据库密码',
          field: 'tenantAppVersion.dbPassword',
          visible: true,
          order: 6
        },
        133: {
          title: '租户应用版本.数据库URL',
          field: 'tenantAppVersion.dbUrl',
          visible: true,
          order: 7
        },
        136: {
      ...
      searchList: [],
      tableId: '1392930032371744773',
      schemeId: '1392930032371744771'
    }
  },
  computed: {},
  methods: {
    getList() {
      ...
      // 数据权限: 公司应用版本gen_company_app
      this.pushDataPermissions(this.search.params, this.$route.meta.routerId, this.tableId)
      remotePage(this.search)
        .then((responseData) => {
        ...
        .then((responseData) => {
          if (responseData.code == 100) {
            this.permission.view = responseData.data.find((item) => {
              return item.permission === 'companyApp:read'
            })
            this.permission.export = responseData.data.find((item) => {
              return item.permission === 'companyApp:export'
            })

            this.permission.add = responseData.data.find((item) => {
              return item.permission === 'companyApp:create'
            })
            this.permission.edit = responseData.data.find((item) => {
              return item.permission === 'companyApp:update'
            })
            this.permission.remove = responseData.data.find((item) => {
              return item.permission === 'companyApp:delete'
            })
          } else {
            this.showMessage(responseData)
          }
