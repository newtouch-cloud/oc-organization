<template>
  <el-row class="dc-container" v-loading="loading">
    ...
    <code-desinge
      ...
      v-on:before-load="setLoad"
      v-on:after-load="resetLoad"
      class="dc-code-desinge_ElImport"
    ></code-desinge>
    ...
    >
      <el-row>
        <el-col :span="6">
          <el-form-item prop="name" label="方案名称" class="dc-el-form-item_SingleInput">
            <el-input
    ...
    >
      ...
      >
        ...
        <template slot-scope="scope">
          ...
          ></OperationIcon>
          <OperationIcon
            v-on:click="onGenerateJavaCode(scope.row)"
            type="primary"
            v-show="permission.edit"
            content="后台生成"
            placement="bottom"
            icon-name="el-icon-caret-right"
            class="dc-OperationIcon_IconButton"
          ></OperationIcon>
          <OperationIcon
...
import { getDcSchemeById, listDcSchemePage, deleteDcScheme } from '@/api/dc/dcScheme'
...
import CodeDesinge from './codeDesinge'
  ...
  methods: {
    ...
    async pageInit() {
      ...
      try {
        ...
        // 数据权限: 低代码-代码方案dc_scheme
        this.pushDataPermissions(this.search.params, this.$route.meta.routerId, this.tableId)
        let [listDcSchemeRespData, listPermissionRespData] = await Promise.all([
          listDcSchemePage(this.search),
          listResourcePermission(this.$route.meta.routerId)
        ])
        if (listDcSchemeRespData.code == 100 && listPermissionRespData.code == 100) {
          this.tableDataTotal = listDcSchemeRespData.data.total
          this.tableData = listDcSchemeRespData.data.rows ? listDcSchemeRespData.data.rows : []
          this.permission.view = listPermissionRespData.data.find((item) => {
            return item.permission === 'dcScheme:read'
          })
          this.permission.export = listPermissionRespData.data.find((item) => {
            return item.permission === 'dcScheme:export'
          })

          this.permission.add = listPermissionRespData.data.find((item) => {
            return item.permission === 'dcScheme:create'
          })
          this.permission.edit = listPermissionRespData.data.find((item) => {
            return item.permission === 'dcScheme:update'
          })
          this.permission.remove = listPermissionRespData.data.find((item) => {
            return item.permission === 'dcScheme:delete'
          })
        } else {
          this.showMessage(listPermissionRespData.code != 100 ? listPermissionRespData : listDcSchemeRespData)
        }
        this.resetLoad()
      } catch (error) {
        this.outputError(error)
    ...
    onGenerateJavaCode(row) {
      this.setLoad()
      getDcSchemeById(row.id)
        .then((responseData) => {
          if (responseData.code == 100) {
            let schema = responseData.data
            if (!validatenull(schema.dcTableList)) {
              for (let i = 0; i < schema.dcTableList.length; i++) {
                this.$set(schema.dcTableList[i], 'constraintList', JSON.parse(schema.dcTableList[i].constraintJson))
                for (let j = 0; j < schema.dcTableList[i].columnList.length; j++) {
                  ...
                  if (!validatenull(childFields) && !validatenull(JSON.parse(childFields))) {
                    JSON.parse(childFields).forEach((item) => {
                      childFieldKeys.push(item.value)
                    })
                  }
                  this.$set(schema.dcTableList[i].columnList[j], 'childFieldKeys', childFieldKeys) // 关联表字段
                }
              }
            }
            this.currentRow = schema
            if (validatenull(schema.template) || validatenull(schema.template.id)) {
              this.$refs.tplDialog.$emit('selectTemplate', schema) // 选择模板
            } else {
              this.$refs.codeDesinge.$emit('openGenerateDcSchemeDialog', this.currentRow)
            }
          } else {
            this.showMessage(responseData)
          }
    ...
    afterChangeTpl(javaTpl, dbModel) {
      this.$refs.codeDesinge.$emit('openAddGenerateDcSchemeDialog', javaTpl, dbModel, this.currentRow)
    },
    // 初始化自定义类型选择框选项变量
    initOptions(This) {}
  },
