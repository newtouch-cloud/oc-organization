...
import { listDcTemplatePage, deleteDcTemplate } from '@/api/dc/dcTemplate'
...
export default {
  ...
  data() {
    return {
        ...
        params: [
          {
            columnName: 'type',
            queryType: '=',
            value: this.$route.params.typeParam
          }
        ],
      ...
      searchList: [],
      tableId: '1338205620666941461',
      schemeId: '1338205620666941459'
    }
  },
  computed: {},
  methods: {
    getList() {
      ...
      listDcTemplatePage(this.search)
        .then((responseData) => {
    ...
    async pageInit() {
      ...
      try {
        ...
        // 数据权限: 低代码-模板dc_template
        this.pushDataPermissions(this.search.params, this.$route.meta.routerId, this.tableId)
        let [listDcTemplateRespData, listPermissionRespData] = await Promise.all([
          listDcTemplatePage(this.search),
          listResourcePermission(this.$route.meta.routerId)
        ])
        if (listDcTemplateRespData.code == 100 && listPermissionRespData.code == 100) {
          this.tableDataTotal = listDcTemplateRespData.data.total
          this.tableData = listDcTemplateRespData.data.rows ? listDcTemplateRespData.data.rows : []
          this.permission.view = listPermissionRespData.data.find((item) => {
            return item.permission === 'dcTemplate:read'
          })
          this.permission.export = listPermissionRespData.data.find((item) => {
            return item.permission === 'dcTemplate:export'
          })

          this.permission.add = listPermissionRespData.data.find((item) => {
            return item.permission === 'dcTemplate:create'
          })
          this.permission.edit = listPermissionRespData.data.find((item) => {
            return item.permission === 'dcTemplate:update'
          })
          this.permission.remove = listPermissionRespData.data.find((item) => {
            return item.permission === 'dcTemplate:delete'
          })
        } else {
          this.showMessage(listPermissionRespData.code != 100 ? listPermissionRespData : listDcTemplateRespData)
        }
        this.resetLoad()
      } catch (error) {
        this.outputError(error)
