  ...
  <el-row class="dc-container" v-loading="loading">
    ...
    >
      <el-row>
        ...
        <el-col :span="6">
          <el-form-item prop="scheme" label="后端方案" class="dc-el-form-item_SelectInput">
            <el-select
...
import { listDcClienSchemePage, deleteDcClienScheme } from '@/api/dc/dcClienScheme'
...
export default {
  ...
  data() {
    return {
      ...
      searchList: [],
      tableId: '1340971493706891267',
      schemeId: '1340971493706891265'
    }
  },
  computed: {},
  methods: {
    getList() {
      ...
      // 数据权限: 前端-代码方案dc_clien_scheme
      this.pushDataPermissions(this.search.params, this.$route.meta.routerId, this.tableId)
      listDcClienSchemePage(this.search)
        .then((responseData) => {
    ...
    async pageInit() {
      ...
      try {
        ...
        // 数据权限: 前端-代码方案dc_clien_scheme
        this.pushDataPermissions(this.search.params, this.$route.meta.routerId, this.tableId)
        let [listDcClienSchemeRespData, listPermissionRespData] = await Promise.all([
          listDcClienSchemePage(this.search),
          listResourcePermission(this.$route.meta.routerId)
        ])
        if (listDcClienSchemeRespData.code == 100 && listPermissionRespData.code == 100) {
          this.tableDataTotal = listDcClienSchemeRespData.data.total
          this.tableData = listDcClienSchemeRespData.data.rows ? listDcClienSchemeRespData.data.rows : []
          this.permission.view = listPermissionRespData.data.find((item) => {
            return item.permission === 'dcClienScheme:read'
          })
          this.permission.export = listPermissionRespData.data.find((item) => {
            return item.permission === 'dcClienScheme:export'
          })

          this.permission.add = listPermissionRespData.data.find((item) => {
            return item.permission === 'dcClienScheme:create'
          })
          this.permission.edit = listPermissionRespData.data.find((item) => {
            return item.permission === 'dcClienScheme:update'
          })
          this.permission.remove = listPermissionRespData.data.find((item) => {
            return item.permission === 'dcClienScheme:delete'
          })
        } else {
          this.showMessage(listPermissionRespData.code != 100 ? listPermissionRespData : listDcClienSchemeRespData)
        }
        this.resetLoad()
      } catch (error) {
        this.outputError(error)
    ...
    onCreate() {
      this.$refs.tplDialog.$emit('selectTemplate')
    },
    onEdit(scope) {
    ...
    afterChangeTpl(clientTpl, dbModel) {
      this.$refs.editForm.$emit('openAddDialog', clientTpl, dbModel)
    },
    onDelete(scope) {
